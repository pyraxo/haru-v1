...
class Hearthstone extends BaseCommand {
  ...
  handle () {
    ...
    this.responds(/^(hearthstone|hs) newpack gvg$/i, matches => {
      this.getCardSet('Goblins vs Gnomes')
    })

    this.responds(/^(hearthstone|hs) newpack wotog$/i, matches => {
      this.getCardSet('Whispers of the Old Gods')
    })

    this.responds(/^(hearthstone|hs) newpack tgt$/i, matches => {
