...
class RipGM extends BaseCommand {
  ...
  createImage (input, year) {
    ...
    year = year || '2016'
    year += ' - 2016'
    gm(path.join(__dirname, 'img/rip.png'))
    .font(path.join(__dirname, 'img/comicsans.ttf'), 23.5)
    .gravity('Center')
    ...
    .write(path.join(__dirname, `img/res-${this.message.id}.png`), err => {
      ...
      this.upload(
        path.join(__dirname, `img/res-${this.message.id}.png`), 'rip.png'
      )
      .then(() => {
  ...
  altImage (user) {
    ...
    base64.base64encoder(user.avatarURL, { string: true }, (err, image) => {
      ...
      fs.readFile(path.join(__dirname, 'img/rip2.png'), (err, src) => {
        if (err) {
        ...
        let out = fs.createWriteStream(
          path.join(__dirname, `img/res-${this.message.id}.png`)
        )
